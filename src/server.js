...
// ─── Flat alias routes (tương thích với frontend cũ) ────────────────────────
...
app.use('/api/info',           (req, res, next) => { req.url = '/info' + req.path;  marketRoutes(req, res, next); });  // FIX: /api/info/SHB → /info/SHB
...
app.use('/api/stock-chat',         (req, res, next) => { req.url = '/stock-chat' + req.path;      aiRoutes(req, res, next); });  // FIX: missing alias
app.use('/api/debug-feed',         (req, res, next) => { req.url = '/debug-feed' + req.path;      aiRoutes(req, res, next); });  // FIX: missing alias
...
app.use('/api/deriv-news',   (req, res, next) => {
    // /api/deriv-news → /news  |  /api/deriv-news/refresh → /news/refresh
