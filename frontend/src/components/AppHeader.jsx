...
<div className="w-full max-w-xl relative z-[99999]">
    ...
    {activeMode !== 'CRYPTO' ? (
        <div className={`flex items-center h-12 border rounded-2xl px-4 focus-within:border-yellow-400/50 transition-all ${UI.searchBg}`}>
            ...
            <input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter') { activeMode === 'PAPER_TRADING' ? executePaperSearch(input) : fetchMarketData(); } }}
                onFocus={() => setShowSuggestions(true)}
                disabled={loadingMarket || activeMode === 'VN_DERIVATIVES'} 
            />
            <button onClick={() => { activeMode === 'PAPER_TRADING' ? executePaperSearch(input) : fetchMarketData(); }} className="h-8 px-6 rounded-lg bg-yellow-400 hover:bg-yellow-300 text-black font-black text-xs transition-all active:scale-95 disabled:opacity-50" disabled={loadingMarket || !input}>SEARCH</button>
        </div>
    ) : (
