...
export default function TradingChart({ data, theme, onIntervalChange, currentInterval }) {
 ...
 const [activeOverlay, setActiveOverlay] = useState(null);
  ...
  // 🚀 TỰ XÂY LÕI QUẢN LÝ VẼ (KHÔNG DÙNG CỦA THƯ VIỆN NỮA)
 const drawingModeRef = useRef(null);
  ...
  const handleFinishText = (rawText) => {
    ...
    // 🚀 Cache cứng ID tránh React async state làm mất reference
    const targetId = inlineInput.id;

    const finalText = rawText?.trim();

    // 🚀 Commit text vào overlay
    if (finalText) {
...
const handleScrollLeft = () => {
  ...
  useEffect(() => {
    ...
    if (!chartInstance.current) {
      ...
      omniChartRef = chartInstance.current;
...
// ==========================================================
  ...
  const toggleIndicator = (name, isMain) => {
    ...
    const handleKeyDown = (e) => {
        // 🚀 KIỂM TRA: Nếu đang gõ trong ô Input thì cấm không cho xóa Overlay!
...
if (!point || !omniChartRef) return [];

const px = omniChartRef.convertToPixel(
    [point],
    { paneId: 'candle_pane' }
);
...
{console.log("INLINE INPUT =", inlineInput)}
          {/* 🚀 KHUNG MỜ GÕ CHỮ */}
          {inlineInput && (
              <input
                 autoFocus
